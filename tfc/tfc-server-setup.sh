#!/bin/bash
#
# Team Fortress Classic Dedicated Server Setup (Docker)
# Builds and runs a TFC server in a Docker container
#
...
readonly GAME_NAME="Team Fortress Classic"
readonly CONTAINER_NAME="tfcserver"
readonly IMAGE_NAME="gameservers/tfc"
readonly STEAM_APP_ID="20"
readonly INSTALL_DIR="/home/steam/tfcserver"

# Game server settings
readonly SERVER_NAME="Silverware TFC Server"
readonly DEFAULT_MAP="2fort"
...
RUN mkdir -p /home/steam/tfcserver && chown -R steam:steam /home/steam/tfcserver

USER steam

RUN /home/steam/steamcmd/steamcmd.sh \
    +force_install_dir /home/steam/tfcserver \
    +login anonymous \
    +app_update 20 validate \
    +quit

WORKDIR /home/steam/tfcserver

EXPOSE 27015/tcp 27015/udp

ENTRYPOINT ["./hlds_run", "-game", "tfc"]
EOF
}
...
setup_data_directory() {
    ...
    DATA_DIR=$(create_game_data_dir "tfc")
    mkdir -p "${DATA_DIR}/tfc"

    local server_cfg="${DATA_DIR}/tfc/server.cfg"
    if [[ ! -f "$server_cfg" ]]; then
        log_info "Creating default server.cfg..."
        cat > "$server_cfg" << EOF
// TFC Server Configuration
...
run_container() {
    ...
    local volume_mappings="-v ${DATA_DIR}/tfc:${INSTALL_DIR}/tfc"
