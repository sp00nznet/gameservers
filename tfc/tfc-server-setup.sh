#!/bin/bash
#
# Team Fortress Classic Dedicated Server Setup
# Downloads, configures, and runs a TFC server using SteamCMD
#
...
readonly SERVICE_NAME="tfcserver"
readonly STEAM_APP_ID="20"
readonly INSTALL_DIR="/opt/tfcserver"
...
ExecStart=/usr/bin/screen -dmS "${SCREEN_NAME}" ${SERVER_BINARY} -game tfc +map ${DEFAULT_MAP} +maxplayers ${MAX_PLAYERS} +port ${GAME_PORT} +sv_lan 0 +rcon_password "${RCON_PASSWORD}"
...
show_summary() {
    ...
    echo -e "  ${DIM}Server config:${RESET}   ${INSTALL_DIR}/tfc/server.cfg"
    echo ""
    separator "-" 60
    echo ""
