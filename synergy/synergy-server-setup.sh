#!/bin/bash
#
# Synergy Dedicated Server Setup (Docker)
# Builds and runs a Synergy co-op server in a Docker container
#
...
readonly GAME_NAME="Synergy"
readonly CONTAINER_NAME="synergyserver"
readonly IMAGE_NAME="gameservers/synergy"
readonly STEAM_APP_ID="17520"
readonly INSTALL_DIR="/home/steam/synergyserver"

# Game server settings
readonly SERVER_NAME="Silverware Synergy Server"
readonly DEFAULT_MAP="d1_trainstation_01"
...
RUN mkdir -p /home/steam/synergyserver && chown -R steam:steam /home/steam/synergyserver

USER steam

RUN /home/steam/steamcmd/steamcmd.sh \
    +force_install_dir /home/steam/synergyserver \
    +login anonymous \
    +app_update 17520 validate \
    +quit

WORKDIR /home/steam/synergyserver

EXPOSE 27015/tcp 27015/udp

ENTRYPOINT ["./srcds_run", "-console", "-game", "synergy"]
EOF
}
...
setup_data_directory() {
    ...
    DATA_DIR=$(create_game_data_dir "synergy")
    mkdir -p "${DATA_DIR}/synergy/cfg"

    local server_cfg="${DATA_DIR}/synergy/cfg/server.cfg"
...
run_container() {
    ...
    local volume_mappings="-v ${DATA_DIR}/synergy/cfg:${INSTALL_DIR}/synergy/cfg"
...
show_summary() {
    ...
    separator "-" 60
    echo -e "${BOLD}About Synergy:${RESET}"
    echo -e "  ${DIM}Synergy is a co-op mod for Half-Life 2 and Episodes.${RESET}"
    echo -e "  ${DIM}Players need the base HL2 game and episodes to play.${RESET}"
    separator "-" 60
