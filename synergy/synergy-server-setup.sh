#!/bin/bash
#
# Synergy Dedicated Server Setup
# Downloads, configures, and runs a Synergy co-op server using SteamCMD
#
...
readonly SERVICE_NAME="synergyserver"
readonly STEAM_APP_ID="17520"
readonly INSTALL_DIR="/opt/synergyserver"
...
ExecStart=/usr/bin/screen -dmS "${SCREEN_NAME}" ${SERVER_BINARY} -console -game synergy +map ${DEFAULT_MAP} +maxplayers ${MAX_PLAYERS} +port ${GAME_PORT} +sv_lan 0 +rcon_password "${RCON_PASSWORD}"
...
show_summary() {
    ...
    echo -e "  ${DIM}Server config:${RESET}   ${INSTALL_DIR}/synergy/cfg/server.cfg"
    echo ""
