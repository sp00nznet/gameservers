#!/bin/bash
#
# Half-Life Deathmatch Dedicated Server Setup
# Downloads, configures, and runs an HLDM server using SteamCMD
#
...
readonly SERVICE_NAME="hldmserver"
readonly STEAM_APP_ID="90"
readonly INSTALL_DIR="/opt/hldmserver"
...
ExecStart=/usr/bin/screen -dmS "${SCREEN_NAME}" ${SERVER_BINARY} -game valve +map ${DEFAULT_MAP} +maxplayers ${MAX_PLAYERS} +port ${GAME_PORT} +sv_lan 0 +rcon_password "${RCON_PASSWORD}"
...
show_summary() {
    ...
    echo -e "  ${DIM}Server config:${RESET}   ${INSTALL_DIR}/valve/server.cfg"
    echo ""
    separator "-" 60
    echo ""
