#!/bin/bash
#
# Half-Life Deathmatch Dedicated Server Setup (Docker)
# Builds and runs an HLDM server in a Docker container
#
...
readonly GAME_NAME="Half-Life Deathmatch"
readonly CONTAINER_NAME="hldmserver"
readonly IMAGE_NAME="gameservers/hldm"
readonly STEAM_APP_ID="90"
readonly INSTALL_DIR="/home/steam/hldmserver"

# Game server settings
readonly SERVER_NAME="Silverware HLDM Server"
readonly DEFAULT_MAP="crossfire"
...
RUN mkdir -p /home/steam/hldmserver && chown -R steam:steam /home/steam/hldmserver

USER steam

RUN /home/steam/steamcmd/steamcmd.sh \
    +force_install_dir /home/steam/hldmserver \
    +login anonymous \
    +app_update 90 validate \
    +quit

WORKDIR /home/steam/hldmserver

EXPOSE 27015/tcp 27015/udp

ENTRYPOINT ["./hlds_run", "-game", "valve"]
EOF
}
...
setup_data_directory() {
    ...
    DATA_DIR=$(create_game_data_dir "hldm")
    mkdir -p "${DATA_DIR}/valve"

    local server_cfg="${DATA_DIR}/valve/server.cfg"
    if [[ ! -f "$server_cfg" ]]; then
        log_info "Creating default server.cfg..."
        cat > "$server_cfg" << EOF
// HLDM Server Configuration
...
run_container() {
    ...
    local volume_mappings="-v ${DATA_DIR}/valve:${INSTALL_DIR}/valve"
